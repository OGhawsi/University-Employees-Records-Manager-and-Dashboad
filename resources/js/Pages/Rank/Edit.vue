      ...
      <div class="flex justify-between my-10">
        ...
        <div class="w-full flex-shrink px-8 items-center">
          ...
          <div class="">
            ...
            <div v-if="errors.changed_rank_id" class="form-error">
              {{ errors.changed_rank_id }}
            </div>
...
export default {
  ...
  data() {
	...
	deleting:false,
      form: this.$inertia.form({
        changed_rank_id: this.rank.rank_id, //intermediate table rank
        same_rank_id: this.rank.rank_id, //intermediate table rank
        subject_of_academic_research: this.rank.subject_of_academic_research,
        registration_of_academic_research: this.rank.registration_of_academic_research,
        completion_of_academic_research: this.rank.completion_of_academic_research,
        current: true,
      }),
    };
  },
  methods: {
    submit() {
      this.form.put(route("rank.update", this.employee.id), {
        preserverState: true,
        preserveScroll: true,
    ...
    remove() {
      if(confirm("are you sure?")) {
        this.$inertia.delete(route('rank.delete', this.rank.id), {
